...
##Clock signal
# set_property -dict { PACKAGE_PIN K17   IOSTANDARD LVCMOS33 } [get_ports { CK }]; #IO_L12P_T1_MRCC_35 Sch=sysclk
# create_clock -add -name CLK -period 8.00 -waveform {0.0 4.0} [get_ports { CK }];


# [DRC LUTLP-1] Combinatorial Loop Alert: 5 LUT cells form a combinatorial loop. This can create a race condition. Timing analysis may not be accurate.
# The preferred resolution is to modify the design to remove combinatorial logic loops.
# If the loop is known and understood, this DRC can be bypassed by acknowledging the condition and setting the following XDC constraint
# on any one of the nets in the loop: 'set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets <myHier/myNet>]'. One net in the loop is Z0. Please evaluate your design. The cells in the loop are: xinv0, xinv1, xinv2, xinv3, and xinv4.

# set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets {Z0 Z1 Z2 Z3 Z4 Z5 Z6 Z7}]

set_property -dict { PACKAGE_PIN K17   IOSTANDARD LVCMOS33 } [get_ports { CLK125 }]; #IO_L12P_T1_MRCC_35 Sch=sysclk
create_clock           -name CKM -period 8.0 [get_ports { CLK125 }]

# ******************************************************************
# note CLKD is CLK125 / 8 and changes on negative edge of CLK125
# ******************************************************************
set CLKD_PIN    [get_pins -of_objects [get_cells -hierarchical xbufg_clkd]    -filter {DIRECTION == OUT}]
create_generated_clock -name CLKED_CLOCK -source  [get_ports CLK125] -edges {2 10 18} [get_pins $CLKD_PIN]
...
set_false_path -from [get_ports GRST]
...
### set_property -dict { PACKAGE_PIN G15   IOSTANDARD LVCMOS33 } [get_ports { GCTRL[0] }]; #IO_L19N_T3_VREF_35 Sch=sw[0]
### set_property -dict { PACKAGE_PIN P15   IOSTANDARD LVCMOS33 } [get_ports { GCTRL[1] }]; #IO_L24P_T3_34 Sch=sw[1]
### set_property -dict { PACKAGE_PIN W13   IOSTANDARD LVCMOS33 } [get_ports { GCTRL[2] }]; #IO_L4N_T0_34 Sch=sw[2]
### set_property -dict { PACKAGE_PIN T16   IOSTANDARD LVCMOS33 } [get_ports { GCTRL[3] }]; #IO_L9P_T1_DQS_34 Sch=sw[3]
...
##Pmod Header JC
#set_property -dict { PACKAGE_PIN V15   IOSTANDARD LVCMOS33     } [get_ports { GREG[31] }]; #IO_L10P_T1_34 Sch=jc_p[1]
#set_property -dict { PACKAGE_PIN W15   IOSTANDARD LVCMOS33     } [get_ports { GREG[30] }]; #IO_L10N_T1_34 Sch=jc_n[1]
#set_property -dict { PACKAGE_PIN T11   IOSTANDARD LVCMOS33     } [get_ports { GREG[29] }]; #IO_L1P_T0_34 Sch=jc_p[2]
#set_property -dict { PACKAGE_PIN T10   IOSTANDARD LVCMOS33     } [get_ports { GREG[28] }]; #IO_L1N_T0_34 Sch=jc_n[2]
...
##Pmod Header JE
#set_property -dict { PACKAGE_PIN V12   IOSTANDARD LVCMOS33 } [get_ports { JACK[0] }]; #IO_L4P_T0_34 Sch=je[1]
#set_property -dict { PACKAGE_PIN W16   IOSTANDARD LVCMOS33 } [get_ports { je[1] }]; #IO_L18N_T2_34 Sch=je[2]
#set_property -dict { PACKAGE_PIN J15   IOSTANDARD LVCMOS33 } [get_ports { JACK[1] }]; #IO_25_35 Sch=je[3]
